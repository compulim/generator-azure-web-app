...
class WebpackConfigurator {
  ...
  enableHotModuleReplacement(port = 80) {
    ...
    this._webpackConfig.module.loaders.forEach(loader => {
      this._webpackConfig.entry.some(entry => loader.test.test(entry)) && loader.loaders.unshift('react-hot')
    });
  ...
  enableWriteToDisk() {
    ...
    this._devServerConfig.outputPath = this._webpackConfig.output.path;
  }
